...
import { resetPassTemp, signUpTemp } from "../../utils/html-templets.js";
...
export const forgetPassword = async (req, res, next) => {
  ...
  const token = jwt.sign(
    { email, forgetCode: hashedCode },
    process.env.RESET_PASSWORS_TOKEN,
    { expiresIn: "10m" }
  );
...
export const resetPassword = async (req, res, next) => {
  ...
  //decode token
  const decode = jwt.verify(token, process.env.RESET_PASSWORS_TOKEN);
  //check email in & forget code in DB
  const user = await User.findOne({
    email: decode?.email,
    forgetCode: decode?.forgetCode,
  });
  if (!user) {
...
export const updatePassword = async (req, res, next) => {
  ...
  return res.status(200).json({
    success: true,
    message: "Password updated successfully, try to login again!",
  });
};
